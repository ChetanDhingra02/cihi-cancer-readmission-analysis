...
The same accreditation statement is reproduced on the dashboard's *Methods, data and licensing*
page and in the footer of each dashboard page.
...
The analysis is observational throughout. Urgent admission describes how an episode began rather
than an assigned treatment, so I treat these results as associations rather than causal effects.
...
I accessed the RAF through the University of Alberta's institutional access under Statistics
Canada's Data Liberation Initiative. The file is licensed rather than public and is not included in
this repository. Reproducing the analysis requires separate access through an eligible institution
and is subject to that institution's terms.

The public repository and deployed dashboard contain no institutional credentials and do not connect
to the restricted CIHI source data.
...
descriptive group rates rather than adjusted site effects. The overall 15.6% is the cohort average
and does not imply the same rate across cancer groups.
...
Urgent index admission was associated with a substantially higher adjusted readmission risk. I
interpret this as an association rather than a causal effect because the RAF only partly captures the
severity and other factors that may be related to both urgent admission and later readmission.
...
The Q2 mortality comparison is more sensitive to the transfer-linkage rule than the Q1 readmission
comparison. The robustness section shows how the estimate changes across the linkage rules.
...
characteristics and after adding COVID coding. The measured characteristics included in these models
did not explain the fitted time association. The observed quarterly series was not a steady rise, and
the analysis does not identify the cause of the change.
...
The logistic model had an AUC of 0.697 in the held-out test set. Its calibration slope was 0.982
and the calibration intercept was -0.135. The decile plot above compares predicted and observed risk
across the ten risk groups. These are internal test-set results and are not evidence of clinical
validation.
...
We used several transfer-linkage rules to measure how much the main Q1 and Q2 results changed with
...
Q2 was more sensitive to the transfer rule. Its risk ratio ranged from 5.41 to 6.55 across the
same rules, compared with 2.44 to 2.52 for Q1.
...
available to this project do not state a numeric minimum cell-size threshold, so this is not presented
as a CIHI-mandated threshold. The dashboard uses the already-cleared tables and does not recalculate
suppression at runtime.
...
The CIHI accreditation statement at the top of this README is part of the project's required
attribution. The analysis, conclusions, opinions and statements are my own.

No open-source code licence is currently declared in this repository. The CIHI Discharge Abstract
Database Research Analytic Files remain separately licensed and are not distributed with this
project.
